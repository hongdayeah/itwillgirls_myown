				...
				<div class="row">
							...
							<div class="table-responsive">
								...
								<table class="table table-bordered table-striped">
									...
									<tr>
										<td>수강정원</td>
										<td>
											${dto.pro_limit}명 <!-- (여유 ${dto.pro_limit - 10}명) --> <!-- 예매한 사람 수만큼 뺴기 -->
										</td>
									</tr>
									<tr>
										<td>수강료</td>
					...
					<div class="post-comment">
						<h3 class="skills">후기 남기기</h3>
							...
							<div class="form-group">
								<div class="col-lg-12">
									<textarea name="rev_con" id="rev_con" rows="8" class=" form-control" placeholder="수강한 프로그램의 후기를 남겨주세요!"></textarea>
								</div>
							</div>
							<p>
...
<script>
	function reviewCheck(){
		...
		let rev_obj = document.getElementById("rev_obj").value;
		let rev_con = document.getElementById("rev_con").value;
		...
		if(confirm(message)){
			$.ajax({
				url: "/program/reviewcreate.do", // 컨트롤러에 대한 URL 매핑
				type: "POST", // 요청 메소드 설정 (POST 또는 GET)
				data: { "rev_obj": rev_obj, "rev_id": rev_id, "rev_con": rev_con, "p_passwd": p_passwd }, // 전송할 데이터 설정
				success: function(response) {
					// 요청이 성공적으로 처리된 후 실행될 콜백 함수
					// 처리 결과에 따른 후속 작업 수행
					//event.preventDefault();
					alert(response);
					//window.location.href = "/program/read.do?pro_obj=" + pro_obj;
				},
